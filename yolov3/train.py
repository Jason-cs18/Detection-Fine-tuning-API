...
#     model.load_state_dict(chkpt['model'])
    ...
    if resume:  # Load previously saved model
        # if the class of new model is equal to the previous model
        if class_num == 0:
            chkpt = torch.load(latest, map_location=device)  # load checkpoint
            model.load_state_dict(chkpt['model'])
            for i, (name, p) in enumerate(model.named_parameters()):
                p.requires_grad = True
        # if the class of new model is not equal to the previous model
        else:
            # load pretrain model (yolov3 from coco)
            model_pretrain = Darknet('cfg/yolov3.cfg', img_size)
            chkpt = torch.load('weights/yolov3.pt', map_location='cpu')
            model_pretrain.load_state_dict(chkpt['model'])
            
            # load customized model (from yolov3.cfg)
            new_model = Darknet(cfg, img_size)
...
#             model = copy.deepcopy(new_model)
        ...
        if transfer_id == 0:
            pass
        # training the High-level Extractor and Detection
        elif transfer_id == 1:
            high_level_extractor = np.load('./high_level_extractors.npy')
            for i, (name, p) in enumerate(model.named_parameters()):
                if name in high_level_extractor or p.shape[0] == nf:
                    p.requires_grad = True
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_num', type=int, default=0, help='identifier to decide whether to change the yolo_layer')
    parser.add_argument('--transfer', action='store_true', help='transfer learning flag')
    parser.add_argument('--transfer_id', type=int, default=0, help='identifier to decide which part will be updated')
